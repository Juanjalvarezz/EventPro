import { getEventByPromotor, getEventsStatus } from "../../utils/eventRequest"
...
function Solicitudes() {
  ...
  return (
    <>
      <h2
        className='bg-gradient-to-r from-complement-800 to-primary-600 montserrat w-fit text-3xl my-6 font-black text-center py-3 px-6  rounded-xl'
      >
            ...
            events.map(event => (
              <div key={event._id} className="montserrat font-bold lg:w-3/5  sm:w-11/12 mb-6 text-xs md:text-sm lg:text-md">
                ...
                <div className="flex justify-center items-center p-2 rounded-b-3xl gap-4 w-full bg-primary-250">
                  ...
                  <button className="bg-red-500 w-36 rounded-xl p-2 sm:text-md md:text-lg lg:text-xl">{`${user.role === 'admin' ? 'Rechazar' : 'Cancelar'}`}</button>
                </div>
              </div>
            ))
          )}
